...
# read files
dataset_path = "../collate_dataset/"
save_path = "./03_result/"
dataset = ["SMRT.csv", "predret.csv"]
set_name = ["SMRT", "Predret"]
smrt, predret = [read_file(dataset[i], dataset_path) for i in range(len(dataset))]
args = get_parser()
if args.load_ckp:
...
smrt.to_csv(os.path.join(save_path, "SMRT_check_temp.csv"), index=False, sep=",")
